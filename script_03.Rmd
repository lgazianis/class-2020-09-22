...
sep_old <- 
  ...
  filter(u_grad > 10) %>%
  group_by(title) %>%
  summarize(total_enrollment = sum(u_grad), .groups="drop")
...
-Wisdom suggests we should define precisely what we want to know in this data set
-We would like an understanding of how online learning is affecting undergraduate enrollment
-We must be thoughtful in grouping our data so that we can view useful trends
...
-We must recognize that we cannot be certain of what the exact cause of each trend is
-Our model may not explain everything factoring into a student's decision to enroll or not enroll
...
```{r}
sep_combine <-
  full_join(sep_old, sep_new, c("title")) %>%
  mutate(change_in_enrollment = total_enrollment.y - total_enrollment.x) %>%
